...
public class RavenousTempleBeastEntity extends AlienEntity {
    ...
    public RavenousTempleBeastEntity(EntityType<? extends AlienEntity> entityType, Level level) {
        super(entityType, level, GigMeleeAttackSelector.NORMAL_ANIM_SELECTOR, Type.RAVENOUS_TEMPLEBEAST);
    }

    public static AttributeSupplier.Builder createAttributes() {
        return LivingEntity.createLivingAttributes()
            .add(Attributes.MAX_HEALTH, CommonMod.config.entityConfigs.ravenoustemplebeastConfigs.ravenoustemplebeastHealth)
            .add(Attributes.ARMOR, CommonMod.config.entityConfigs.ravenoustemplebeastConfigs.ravenoustemplebeastArmor)
            .add(Attributes.ARMOR_TOUGHNESS, 0.0)
            .add(Attributes.KNOCKBACK_RESISTANCE, 0.0)
            .add(Attributes.FOLLOW_RANGE, 16.0)
            .add(Attributes.MOVEMENT_SPEED, 0.3300000041723251)
            .add(Attributes.ATTACK_DAMAGE, CommonMod.config.entityConfigs.ravenoustemplebeastConfigs.ravenoustemplebeastAttackDamage)
